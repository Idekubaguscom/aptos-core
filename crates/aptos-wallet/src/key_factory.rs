...
//! AptosWallet.
//!
//! Note that the Aptos Blockchain makes use of ed25519 Edwards Digital Signature Algorithm
...
impl ExtendedPrivKey {
    ...
    pub fn get_address(&self) -> AccountAddress {
        aptos_types::account_address::from_public_key(&self.get_public())
    }
    ...
    /// Aptos specific sign function that is capable of signing an arbitrary
    /// Serializable value.
    ///
    /// NOTE: In Aptos, we do not sign the raw bytes of a transaction, but
    ...
    /// described in `aptos_crypto::hash`.
    ///
...
impl KeyFactory {
    ...
    pub fn private_child(&self, child: ChildNumber) -> Result<ExtendedPrivKey> {
        // application info in the HKDF context is defined as Aptos derived key$child_number.
        let mut le_n = [0u8; 8];
