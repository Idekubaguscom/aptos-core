...
//! # Tool to help maintaining aptos specifications.
//!
//! '''bash
//! cargo run -p aptos-documentation-tool -- --help
//! '''
...
#[structopt(
    ...
    about = "Tool to help maintaining aptos specifications"
)]
struct Options {
    ...
    #[structopt(long)]
    update_aptos_specs_dir: Option<PathBuf>,
}
...
fn process_specs(dir: PathBuf, definitions: &BTreeMap<String, String>) -> std::io::Result<()> {
    for entry in std::fs::read_dir(dir.as_path())? {
        ...
        let output = aptos_doc::update_rust_quotes(file, definitions)?;
...
fn main() {
    ...
    let definitions = aptos_doc::quote_container_definitions(&registry)
        .expect("generating definitions should not fail");

    match options.update_aptos_specs_dir {
