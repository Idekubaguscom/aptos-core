...
fn publish_metrics_test() {
    let counter_opts = Opts::new("aptos_test_counter", "aptos test counter help");
    ...
    assert_eq!("aptos test counter help", m.get_help());
    assert_eq!("aptos_test_counter", m.get_name());
