...
/// A builder for a `AptosLogger`, configures what, where, and how to write logs.
pub struct AptosLoggerBuilder {
    channel_size: usize,
    level: Level,
    remote_level: Level,
...
impl AptosLoggerBuilder {
    ...
    pub fn build(&mut self) -> Arc<AptosLogger> {
        let filter = {
            let local_filter = {
        ...
        let logger = if self.is_async {
            ...
            let logger = Arc::new(AptosLogger {
                sender: Some(sender),
                printer: None,
...
pub struct AptosLogger {
    sender: Option<SyncSender<LoggerServiceEvent>>,
    printer: Option<Box<dyn Writer>>,
    filter: RwLock<AptosFilter>,
    pub(crate) formatter: fn(&LogEntry) -> Result<String, fmt::Error>,
}

impl AptosLogger {
    pub fn builder() -> AptosLoggerBuilder {
        AptosLoggerBuilder::new()
    }

    #[allow(clippy::new_ret_no_self)]
    pub fn new() -> AptosLoggerBuilder {
        Self::builder()
    }
...
/// or to a `AptosLogger` for external logging.
struct LoggerService {
    receiver: Receiver<LoggerServiceEvent>,
    address: Option<String>,
    printer: Option<Box<dyn Writer>>,
    facade: Arc<AptosLogger>,
}
