...
use state::State;
pub mod types;
pub use types::{AptosAccount, Resource, RestError};
pub mod views;
...
impl Client {
    ...
    cfg_dpn! {
        ...
        // Returns root account AptosAccount::Config<AptosVersion> resource
        pub async fn get_aptos_version(&self) -> Result<Response<dpn::AptosConfig<dpn::AptosVersion>>> {
            self.get_resource::<dpn::AptosConfig<dpn::AptosVersion>>(dpn::aptos_root_address(), &dpn::config_struct_tag(dpn::aptos_version_identifier())).await
        }

        // Returns root account AptosAccount::Configuration resource
    ...
    pub async fn get_account(&self, address: AccountAddress) -> Result<Response<AptosAccount>> {
