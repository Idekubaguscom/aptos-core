...
use aptos_transaction_builder::{error_explain, stdlib::ScriptCall};
use aptos_types::{
    account_config::{
        ...
        DesignatedDealerPreburns, AptosAccountResource, FreezingBit, Limit, MintEvent,
    ...
    aptos_id_identifier::AptosIdVaspDomainIdentifier,
    event::EventKey,
    proof::{
...
pub enum AccountRoleView {
    ...
    ParentVASP {
        ...
        vasp_domains: Option<Vec<AptosIdVaspDomainIdentifier>>,
    },
...
pub enum EventDataView {
    ...
    VASPDomain {
        ...
        domain: AptosIdVaspDomainIdentifier,
...
impl MetadataView {
    ...
    pub fn with_aptos_root(&mut self, aptos_root: &AccountState) -> Result<()> {
        if let Some(vm_publishing_option) = aptos_root.get_vm_publishing_option()? {
        ...
        if let Some(aptos_version) = aptos_root.get_aptos_version()? {
            self.aptos_version = Some(aptos_version.major);
        }
