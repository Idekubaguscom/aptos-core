...
#[structopt(
    name = "swiss-knife",
    about = "Tool for generating, serializing (BCS), hashing and signing Aptos transactions. Additionally, contains tools for testing. Please refer to README.md for examples."
)]
struct Opt {
...
struct GenerateKeypairResponse {
    ...
    pub public_key: String,
    pub aptos_auth_key: String,
    pub aptos_account_address: String,
}
...
fn generate_key_pair(seed: Option<u64>) -> GenerateKeypairResponse {
    ...
    let aptos_auth_key = AuthenticationKey::ed25519(&keypair.public_key);
    let aptos_account_address: String = aptos_auth_key.derived_address().to_string();
    let aptos_auth_key: String = aptos_auth_key.to_string();
