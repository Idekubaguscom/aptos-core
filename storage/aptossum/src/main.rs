...
fn run_cmd() -> Result<()> {
    ...
    match opt.cmd {
        Command::Status => {
            print(aptossum.get_startup_info()?, is_json)?;
        }
        Command::Txn(txn_op) => match txn_op {
            TxnOp::Get { version } => {
                print(&aptossum.get_txn_by_version(version)?, is_json)?;
            }
            TxnOp::Scan {
                from_version,
                to_version,
                ...
                let to_version = match to_version {
                    ...
                    None => aptossum.get_committed_version()?,
                };
                print(
                    aptossum.scan_txn_by_version(from_version, to_version)?,
                    is_json,
                )?;
            }
        },
        ...
        Command::Event(event_cmd) => match event_cmd {
            EventCmd::EventByKey {
                ...
                EventOp::Get { seq } => {
                    print(aptossum.scan_events_by_seq(&event_key, seq, seq)?, is_json)?;
                }
                EventOp::Scan { from_seq, to_seq } => {
                    ...
                    print(
                        aptossum.scan_events_by_seq(&event_key, from_seq, to_seq)?,
                        is_json,
                    )?;
                }
            },
            EventCmd::EventByVersion { version } => {
                print(aptossum.get_events_by_version(version)?, is_json)?;
            }
        },
    }
    Ok(())
