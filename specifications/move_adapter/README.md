...
by the `GasConstants` structure in the `AptosVMConfig` module.
...
function in the `AptosTransactionPublishingOption` module to determine if the script
...
`0x1::AptosTransactionPublishingOption::AptosTransactionPublishingOption.script_allowlist`.
...
the `AptosTransactionPublishingOption` module to see if publishing is allowed
...
function from the `AptosBlock` module in the Aptos Framework to record the
