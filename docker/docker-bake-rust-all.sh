...
GIT_REV=$(git rev-parse --short=8 HEAD) GIT_SHA1=$(git rev-parse HEAD) BUILD_DATE="$(date -u +'%Y-%m-%dT%H:%M:%SZ')" docker buildx bake --push --file docker/docker-bake-rust-all.hcl all
