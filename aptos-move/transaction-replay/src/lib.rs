...
use aptos_validator_interface::{DBDebuggerInterface, DebuggerStateView, AptosValidatorInterface};
use aptos_vm::{
    ...
    pub fn execute_writeset_at_version(
            ...
            .get_account_state_by_version(aptos_root_address(), base_version)?
        {
            Some(account) => account
                ...
                .ok_or_else(|| anyhow!("Aptos root account doesn't exist"))?
                .sequence_number(),
            None => bail!("Aptos root account blob doesn't exist"),
        };
