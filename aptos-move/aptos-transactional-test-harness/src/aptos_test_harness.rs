...
use aptos_types::{
    ...
    account_config::{
        ...
        type_tag_for_currency_code, BalanceResource, AptosAccountResource, XUS_IDENTIFIER, XUS_NAME,
    },
...
/// production settings:
///   - It uses a Aptos StateView as its storage backend
...
/// Parameters *required* to create a Aptos transaction.
...
/// Custom commands for the Aptos transactional test flow.
#[derive(StructOpt, Debug)]
...
fn diem_framework_private_key_mapping() -> Vec<(String, Ed25519PrivateKey)> {
    vec![
        ("AptosRoot".to_string(), GENESIS_KEYPAIR.0.clone()),
...
impl<'a> AptosTestAdapter<'a> {
    ...
    fn fetch_account_resource(&self, signer_addr: &AccountAddress) -> Result<AptosAccountResource> {
        let account_access_path = AccessPath::resource_access_path(ResourceKey::new(
            *signer_addr,
            AptosAccountResource::struct_tag(),
        ));
    ...
    /// Perform a single Aptos transaction.
    ///
...
impl<'a> MoveTestAdapter<'a> for AptosTestAdapter<'a> {
        ...
        }) = task_opt
        {
            ...
            if let Some(validators) = init_args.validators {
                for validator_name in validators {
                    ...
                    // This is because we need to fetch the sequence number of AptosRoot, which is
    ...
    fn execute_script(
        ...
        if !extra_args.admin_script {
            panic!(
                "Transactions scripts are not currently allowed on Aptos. \
...
/// Run the Aptos transactional test flow, using the given file as input.
pub fn run_test(path: &Path) -> Result<(), Box<dyn std::error::Error>> {
    run_test_impl::<AptosTestAdapter>(path, Some(&*PRECOMPILED_DIEM_FRAMEWORK))
}
