...
use diem_framework::aptos_stdlib_files;
use aptos_types::on_chain_config::{AptosVersion, DIEM_MAX_KNOWN_VERSION};
...
pub fn main() -> Result<()> {
    ...
    let flags = ReplayFlags {
        aptos_version: args
            .aptos_version
            .map_or(DIEM_MAX_KNOWN_VERSION, |v| AptosVersion { major: v }),
        filters,
    ...
    let env = run_model_builder(&aptos_stdlib_files(), &[])?;
