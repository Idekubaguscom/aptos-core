...
use aptos_types::{
    ...
    on_chain_config::{
        AptosVersion, OnChainConfig, ParallelExecutionConfig, VMConfig, VMPublishingOption,
        ...
        // Run the execution logic
        {
            ...
            match payload {
                TransactionPayload::Script(script) => {
                    let aptos_version = self.0.get_aptos_version()?;
                    let remapped_script =
    ...
    fn execute_writeset<S: MoveResolver>(
        ...
        Ok(match writeset_payload {
            ...
            WriteSetPayload::Script { script, execute_as } => {
                ...
                let aptos_version = self.0.get_aptos_version().map_err(Err)?;
                let senders = match txn_sender {
                    None => vec![*execute_as],
                    Some(sender) => vec![sender, *execute_as],
                };
                let remapped_script = if aptos_version < aptos_types::on_chain_config::DIEM_VERSION_2
                {
                    None
                } else {
                    script_to_script_function::remapping(script.code())
...
impl VMValidator for AptosVM {
    ...
    /// 3. Invokes `AptosAccount.prologue`, which checks properties such as the transaction has the
...
impl VMAdapter for AptosVM {
    ...
    fn check_transaction_format(&self, txn: &SignedTransaction) -> Result<(), VMStatus> {
        if txn.is_multi_agent() && self.0.get_aptos_version()? < DIEM_VERSION_3 {
    ...
    fn run_prologue<S: MoveResolver>(
        ...
        match transaction.payload() {
            ...
            TransactionPayload::ScriptFunction(_) => {
                // gate the behavior until the Aptos version is ready
                if self.0.get_aptos_version()? < DIEM_VERSION_2 {
                    return Err(VMStatus::Error(StatusCode::FEATURE_UNDER_GATING));
                }
