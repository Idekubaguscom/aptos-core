...
use aptos_types::{
    ...
    event::EventKey,
    on_chain_config::{
        ConfigStorage, AptosVersion, OnChainConfig, VMConfig, VMPublishingOption, DIEM_VERSION_3,
    },
...
impl AptosVMImpl {
    ...
    /// Provides access to some internal APIs of the Aptos VM.
    ...
    fn load_configs_impl<S: ConfigStorage>(&mut self, data_cache: &S) {
        ...
        self.version = AptosVersion::fetch_config(data_cache);
    ...
    pub fn get_aptos_version(&self) -> Result<AptosVersion, VMStatus> {
        self.version.clone().ok_or_else(|| {
            CRITICAL_ERRORS.inc();
            error!("VM Startup Failed. Aptos Version Not Found");
            VMStatus::Error(StatusCode::VM_STARTUP_FAILURE)
        })
    }
    ...
    pub(crate) fn run_script_prologue<S: MoveResolver>(
        ...
        let secondary_public_key_hashes: Vec<MoveValue> = txn_data
            ...
            .collect();
        let args = if self.get_aptos_version()? >= DIEM_VERSION_3 && txn_data.is_multi_agent() {
            vec![
        ...
        let prologue_function_name =
            if self.get_aptos_version()? >= DIEM_VERSION_3 && txn_data.is_multi_agent() {
...
/// Internal APIs for the Aptos VM, primarily used for testing.
#[derive(Clone, Copy)]
...
impl<'a> AptosVMInternals<'a> {
    ...
    pub fn aptos_version(self) -> Result<AptosVersion, VMStatus> {
        self.0.get_aptos_version()
    }
