...
/// * A `AptosAccount::SentPaymentEvent` with the `payer` field being the Parent VASP's address,
/// and payee field being `child_address`. This is emitted on the Parent VASP's
/// `AptosAccount::AptosAccount` `sent_events` handle.
/// * A `AptosAccount::ReceivedPaymentEvent` with the  `payer` field being the Parent VASP's address,
/// and payee field being `child_address`. This is emitted on the new Child VASPS's
/// `AptosAccount::AptosAccount` `received_events` handle.
///
/// # Parameters
...
/// | ----------------            | --------------                                           | -------------                                                                            |
/// | `Errors::INVALID_ARGUMENT`  | `AptosAccount::EMALFORMED_AUTHENTICATION_KEY`            | The `auth_key_prefix` was not of length 32.                                              |
...
/// | `Errors::NOT_PUBLISHED`     | `Aptos::ECURRENCY_INFO`                                  | The `CoinType` is not a registered currency on-chain.                                    |
/// | `Errors::INVALID_STATE`     | `AptosAccount::EWITHDRAWAL_CAPABILITY_ALREADY_EXTRACTED` | The withdrawal capability for the sending account has already been extracted.            |
/// | `Errors::NOT_PUBLISHED`     | `AptosAccount::EPAYER_DOESNT_HOLD_CURRENCY`              | The sending account doesn't have a balance in `CoinType`.                                |
/// | `Errors::LIMIT_EXCEEDED`    | `AptosAccount::EINSUFFICIENT_BALANCE`                    | The sending account doesn't have at least `child_initial_balance` of `CoinType` balance. |
/// | `Errors::INVALID_ARGUMENT`  | `AptosAccount::ECANNOT_CREATE_AT_VM_RESERVED`            | The `child_address` is the reserved address 0x0.                                         |
///
/// # Related Scripts
...
fun create_child_vasp_account<CoinType>(
    ...
    AptosAccount::create_child_vasp_account<CoinType>(
        &parent_vasp,
        child_address,
        auth_key_prefix,
        add_all_currencies,
    ...
    if (child_initial_balance > 0) {
        let vasp_withdrawal_cap = AptosAccount::extract_withdraw_capability(&parent_vasp);
        AptosAccount::pay_from<CoinType>(
            &vasp_withdrawal_cap, child_address, child_initial_balance, x"", x""
        );
        AptosAccount::restore_withdraw_capability(vasp_withdrawal_cap);
    };
}
}
