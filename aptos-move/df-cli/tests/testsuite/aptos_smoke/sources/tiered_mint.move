...
use AptosFramework::SlidingNonce;
...
/// * A `Aptos::MintEvent` with the amount and currency code minted is emitted on the
/// `mint_event_handle` in the stored `Aptos::CurrencyInfo<CoinType>` resource stored under
/// `0xA550C18`; and
...
/// | `Errors::REQUIRES_CAPABILITY` | `Aptos::EMINT_CAPABILITY`                    | `tc_account` does not have a `Aptos::MintCapability<CoinType>` resource published under it.                                  |
/// | `Errors::INVALID_STATE`       | `Aptos::EMINTING_NOT_ALLOWED`                | Minting is not currently allowed for `CoinType` coins.                                                                       |
/// | `Errors::LIMIT_EXCEEDED`      | `AptosAccount::EDEPOSIT_EXCEEDS_LIMITS`      | The depositing of the funds would exceed the `account`'s account limits.                                                     |
///
/// # Related Scripts
