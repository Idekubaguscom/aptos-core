...
use aptos_types::{
    ...
    account_config::{
        type_tag_for_currency_code, BalanceResource, AptosAccountResource, CORE_CODE_ADDRESS,
    },
    block_metadata::{new_block_event_key, BlockMetadata, NewBlockEvent},
    on_chain_config::{
        AptosVersion, OnChainConfig, ParallelExecutionConfig, VMPublishingOption, ValidatorSet,
    },
    transaction::{
...
use aptos_vm::{
    ...
    read_write_set_analysis::add_on_functions_list, AptosVM, VMExecutor, VMValidator,
};
...
impl FakeExecutor {
    ...
    pub fn read_account_resource(&self, account: &Account) -> Option<AptosAccountResource> {
    ...
    pub fn execute_transaction_block_parallel(
        ...
        let (result, _) =
            ParallelAptosVM::execute_block(&analyze_result, txn_block, &self.data_store)?;
