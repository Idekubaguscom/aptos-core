...
pub static CREATE_ACCOUNT_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    let code = "
    import 0x1.Aptos;
    import 0x1.AptosAccount;

    main<Token>(account: signer, fresh_address: address, auth_key_prefix: vector<u8>, initial_amount: u64) {
      let with_cap: AptosAccount.WithdrawCapability;
      let name: vector<u8>;
    label b0:
      name = h\"\";

      AptosAccount.create_parent_vasp_account<Token>(
        &account,
        copy(fresh_address),
    ...
    label b1:
      with_cap = AptosAccount.extract_withdraw_capability(&account);
      AptosAccount.pay_from<Token>(
        &with_cap,
        move(fresh_address),
        move(initial_amount),
        h\"\",
      ...
      AptosAccount.restore_withdraw_capability(move(with_cap));
    label b2:
      return;
    }
";
...
pub static MULTI_AGENT_SWAP_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    main(alice: signer, bob: signer, amount_xus: u64, amount_xdx: u64) {
        ...
        let alice_withdrawal_cap: AptosAccount.WithdrawCapability;
        let bob_withdrawal_cap: AptosAccount.WithdrawCapability;
        let alice_addr: address;
        let bob_addr: address;
    label b0:
        alice_withdrawal_cap = AptosAccount.extract_withdraw_capability(&alice);
        bob_addr = Signer.address_of(&bob);
        AptosAccount.pay_from<XUS.XUS>(
            &alice_withdrawal_cap, move(bob_addr), move(amount_xus), h\"\", h\"\"
        );
        AptosAccount.restore_withdraw_capability(move(alice_withdrawal_cap));

        // Then, Bob pays Alice in currency XDX.
        bob_withdrawal_cap = AptosAccount.extract_withdraw_capability(&bob);
        alice_addr = Signer.address_of(&alice);
        AptosAccount.pay_from<XDX.XDX>(
            &bob_withdrawal_cap, move(alice_addr), move(amount_xdx), h\"\", h\"\"
        );
        AptosAccount.restore_withdraw_capability(move(bob_withdrawal_cap));
        return;
    }
";
...
pub static MULTI_AGENT_MINT_SCRIPT: Lazy<Vec<u8>> = Lazy::new(|| {
    ...
    label b0:
        ...
        AptosAccount.restore_withdraw_capability(move(dd_withdrawal_cap));
        return;
    }
";
