...
main(dr_account: signer, account: signer, auth_key_prefix: vector<u8>) {
  let rotate_cap: AptosAccount.KeyRotationCapability;
label b0:
  rotate_cap = AptosAccount.extract_key_rotation_capability(&account);
  AptosAccount.rotate_authentication_key(&rotate_cap, move(auth_key_prefix));
  AptosAccount.restore_key_rotation_capability(move(rotate_cap));
...
"#;
    ...
    let account = Account::new_aptos_root();
    let txn = account
        .transaction()
...
label b0:
  ...
  AptosConfig.reconfigure(&dr_account);
  return;
}
"#;
