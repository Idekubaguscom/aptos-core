...
fn verify_and_execute_writeset() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        let updated_aptos_root_account = executor
        ...
        assert_eq!(test_env.dr_sequence_number.checked_add(1).unwrap(), updated_aptos_root_account.sequence_number());
...
fn bad_writesets() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        // (1) A WriteSet signed by an arbitrary account, not Aptos root, should be rejected.
        ...
        // (3) A WriteSet attempting to change AptosWriteSetManager should be dropped.
        let key = ResourceKey::new(
            *genesis_account.address(),
            StructTag {
                address: CORE_CODE_ADDRESS,
                module: Identifier::new("AptosAccount").unwrap(),
                name: Identifier::new("AptosWriteSetManager").unwrap(),
                type_params: vec![],
            },
        );
        ...
        // (4) A WriteSet attempting to change Aptos root AccountResource should be dropped.
        let key = ResourceKey::new(
            *genesis_account.address(),
            StructTag {
                address: CORE_CODE_ADDRESS,
                module: Identifier::new("AptosAccount").unwrap(),
                name: Identifier::new("AptosAccount").unwrap(),
                type_params: vec![],
            },
        );
        let path = AccessPath::resource_access_path(key);
