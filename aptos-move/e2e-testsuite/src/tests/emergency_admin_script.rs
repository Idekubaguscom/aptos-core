        ...
        assert!(executor
            .try_exec(
                ...
                serialize_values(&vec![
                    MoveValue::Signer(*aptos_root_account.address()),
...
fn halt_network() {
    ...
    // `AptosTransactionPublishingOption::halt_all_transactions` is not available in version 1.
    test_with_different_versions! {2..=DIEM_MAX_KNOWN_VERSION.major, |test_env| {
        ...
        let auth_key = aptos_root_account.auth_key();
        // AptosRoot can still send transaction
        executor.execute_and_apply(
            aptos_root_account
                .transaction()
