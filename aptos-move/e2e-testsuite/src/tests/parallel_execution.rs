...
use aptos_vm::{parallel_executor::ParallelAptosVM, read_write_set_analysis::add_on_functions_list};
...
fn peer_to_peer_with_prologue_parallel() {
    ...
    let (mut results, parallel_status) =
        ParallelAptosVM::execute_block(&analyze_result, txns, executor.get_state_view()).unwrap();
