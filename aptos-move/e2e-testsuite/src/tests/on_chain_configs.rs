...
fn initial_aptos_version() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        assert_eq!(
            vm.internals().aptos_version().unwrap(),
            AptosVersion { major: test_env.version_number }
        );
        ...
        let txn = account
            .transaction()
            .script(Script::new(
                LegacyStdlibScript::UpdateAptosVersion
                    .compiled_bytes()
                    .into_vec(),
                vec![],
        ...
        assert_eq!(
            new_vm.internals().aptos_version().unwrap(),
            AptosVersion { major: test_env.version_number + 1 }
        );
    }
    }
}
...
fn update_consensus_config() {
    test_with_different_versions! {CURRENT_RELEASE_VERSIONS, |test_env| {
        ...
        if test_env.version_number == 2 {
            ...
            assert!(matches!(executor.execute_transaction(generate_txn(seq_num, "update_aptos_consensus_config", Some(vec![1,2,3]))).status(), &TransactionStatus::Keep(KeptVMStatus::MoveAbort(_, _))));
            assert_eq!(executor.execute_and_apply(generate_txn(seq_num, "initialize_aptos_consensus_config", None)).status(), &TransactionStatus::Keep(KeptVMStatus::Executed));
            assert_eq!(executor.execute_and_apply(generate_txn(seq_num + 1, "update_aptos_consensus_config", Some(vec![1,2,3]))).status(), &TransactionStatus::Keep(KeptVMStatus::Executed));
        }
