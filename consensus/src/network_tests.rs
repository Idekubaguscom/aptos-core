...
pub struct NetworkPlayground {
    ...
    node_consensus_txs: Arc<
        Mutex<HashMap<TwinId, aptos_channel::Sender<(PeerId, ProtocolId), PeerManagerNotification>>>,
    >,
...
mod tests {
    ...
    #[test]
    fn test_bad_message() {
        let (peer_mgr_notifs_tx, peer_mgr_notifs_rx) = aptos_channel::new(QueueStyle::FIFO, 8, None);
